...
def train_Bullet_DDPG(env='HalfCheetahBulletEnv-v0', consec_frames=3, running_stat=False, load_model=False,
    ...
    # set noise
    noise = OUNoise_Exploration(ac_space.shape[0], init_epsilon=0.5, final_epsilon=0.1, explore_len=100000)
    ...
    # set replay memory
    memory = ReplayBuffer(memory_cap=1000000, batch_size_q=128)

    # set data generator
    data_generator = Memory_Data_Generator(agent=agent,
                                           env=env,
                                           memory=memory,
