...
class ES_path_Data_generator:
    def __init__(self,
                 ...
                 action_repeat):
        """
        Generate several whole paths for differently perturbed parameters. This
        is used for the ES Algorithm.

        Args:
            agent: the agent for action selection
            env: the environment
            n_kid: the number of kids
            sigma: how big the perturbation is
            path_num: number of paths to generate at every call
            action_repeat: number of repeated actions
            render: whether display the game or not
        """
        mp.set_start_method('spawn')
        ...
        self.sigma = sigma
        self.scaler = Scaler(env.observation_space_sca.shape)
        for i in range(2 * self.n_kid):
            s = Queue()
            r = Queue()
            self.senders.append(s)
            self.receivers.append(r)
            self.workers.append(mp.Process(target=path_rollout, args=(agent, env, path_num, r, s, render, action_repeat, i)))
        for worker in self.workers:
            worker.start()

    def set_param(self, params):
        """
        Set the parameters for the sub agents. Each agent is assigned with a specific perturbed parameter.
